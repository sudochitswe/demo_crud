...
namespace CBMMIS_WebApi.DataAccess
{
    public class BaseDA
    {
        ...
        public static ResultStatus GetTotalRecord(string table, string filter)
        {
            string queryString = String.Format("SELECT count(1) FROM {0} {1} ", table, filter);
            ...
            try 
            {
                ...
                conn.Open();
                result.Data = Convert.ToInt32(cmd.ExecuteScalar());
                conn.Close();
            }
            catch (Exception exp)
            {
        ...
        public static ResultStatus GetTotalRecord02(string table, string filter)
        {
            string queryString = String.Format("SELECT count(1) FROM {0} {1} ", table, filter);
            ResultStatus result = new ResultStatus();
            try 
        ...
        public static ResultStatus GetTotalRecords(string table)
        {
            string query = String.Format("SELECT count(1) FROM {0}", table);
            ResultStatus result = new ResultStatus();
        ...
        public static string GetQueryStringBySearch02( string searchingColumns, string TemplatedId,string cbm_reference_number,string fromDate,string toDate,string userId,string status,string Event)
        {
            ...
            if (!String.IsNullOrEmpty(TemplatedId))
                ...
                conditions.Add(" " + SearchColum[0] + " = '" + TemplatedId + "' ");
        ...
        public ResultStatus CopyTo(DataTable dataTable, string tableName, string connectionString)
        {
            ...
            if (string.IsNullOrEmpty(connectionString)) connectionString = BaseDA.MainConnectionString;
